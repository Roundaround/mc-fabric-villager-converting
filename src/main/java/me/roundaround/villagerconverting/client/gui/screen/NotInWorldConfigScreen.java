...
public class NotInWorldConfigScreen extends Screen {
  ...
  public NotInWorldConfigScreen(Screen parent) {
    super(Text.translatable("villagerconverting.config.title"));
    this.parent = parent;
  }
  ...
  protected void init() {
    ...
    this.layout.addBody(LabelWidget.builder(this.textRenderer,
        List.of(Text.translatable("villagerconverting.config.notInWorld.1"),
            Text.translatable("villagerconverting.config.notInWorld.2")
        )
